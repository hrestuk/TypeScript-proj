...
// Додав поле id до Lesson для можливості посилатися на конкретне заняття при зміні.
type Lesson = {
  id: number;
  courseId: number;
  professorId: number;
...
function addLesson(lesson: Omit<Lesson, "id">): boolean {
  ...
  const lessonWithId: Lesson = { ...lesson, id: nextLessonId++ };
...
function getMostPopularCourseType(): CourseType | null {
  ...
  const counter: { [K in CourseType]?: number } = {};
...
const conflictLesson: Omit<Lesson, "id"> = {
  courseId: 1,
